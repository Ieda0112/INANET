...
base_split_dir = "datasets/COO"  # 振り分け先ディレクトリ
...
with open(os.path.join(base_split_dir, "train_list.txt"), "w") as f:
    for fname in train_files:
        f.write(fname + "\n")
with open(os.path.join(base_split_dir, "valid_list.txt"), "w") as f:
    for fname in valid_files:
        f.write(fname + "\n")
with open(os.path.join(base_split_dir, "test_list.txt"), "w") as f:
    for fname in test_files:
        f.write(fname + "\n")
