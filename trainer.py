...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            # # 確認のために挿入しているだけ
            # if self.experiment.validation:
            #     avg_valid_loss = self.validate(validation_loaders, model, epoch, self.steps)
            #     # self.logger.info('Epoch %6d avg_valid_loss: %.6f validation: %s' % (epoch, avg_valid_loss, metric_str))
            #     self.logger.info('Epoch %6d avg_valid_loss: %.6f' % (epoch, avg_valid_loss))
            #     with open(self.full_log_path, 'a') as f:
            #         f.write(f'VALID_EPOCH_END\tepoch:{epoch:3d}\tstep:{self.steps:6d}\tavg_valid_loss:{avg_valid_loss:.6f}\n')
    ...
    def train_step(self, model, optimizer, batch, epoch, step, **kwards):
        ...
        # # Debug: show batch keys/types when verbose to catch shape differences
        # if self.logger.verbose:
        #     try:
        #         info_items = []
        #         for k, v in batch.items():
        #             if hasattr(v, 'shape'):
        #                 info_items.append(f"{k}:{type(v).__name__}{getattr(v, 'shape', '')}")
        #             else:
        #                 info_items.append(f"{k}:{type(v).__name__}")
        #         print('train_step batch:', ', '.join(info_items))
        #     except Exception:
        #         pass
    ...
    # # validationに特化した関数、元のvalidateは評価に特化した関数だったため
    # def valid_step(self, model, optimizer, batch, epoch, step, **kwards):
    #     results = model.forward(batch, training=True)
    #     if len(results) == 2:
    #         l, pred = results
    #         metrics = {}
    ...
    def validate(self, validation_loaders, model, epoch, step):
        ...
        all_matircs = {}
        all_valid_losses = []
        # Set model to eval mode for stable BatchNorm/Dropout behavior
        model.eval()
        ...
        for name, loader in validation_loaders.items():
            ...
            if self.experiment.validation.visualize:
                all_valid_losses = self.validate_step(loader, model, True, compute_metrics=False)
                # print(f"valid_losses: {valid_losses}")
            else:
                all_valid_losses = self.validate_step(loader, model, False, compute_metrics=False)
    ...
    def validate_step(self, data_loader, model, visualize=False, compute_metrics=True):
        # Train-like validation: only compute loss, no metrics
        valid_losses = []
        # Disable grad to reduce memory use during validation
        with torch.no_grad():
            for batch in data_loader:
                # print(f"validate_batch:")
                # for k, v in batch.items():
                #     if hasattr(v, 'shape'):
                #         print(f"  {k}: {type(v).__name__}{getattr(v, 'shape', '')}")
                #     else:
                ...
                # print('DEBUG validate: calling model.forward with training=True for loss computation')
                results = model.forward(batch, training=True)
                # print('DEBUG validate: model.forward returned results of length', len(results))
                # train_stepと同じロジックでlossを計算
                if len(results) == 2:
                    l, pred_loss = results
                elif len(results) == 3:
                    l, pred_loss, metrics_loss = results
                else:
                    print(f"validate_step: unexpected results length: {len(results)}")
                    for res in results:
                        print(f"len of result:{len(res)}")
                        print(f"result:{res}")
                
                # -- debug: print `l` info just before loss aggregation --
                # try:
                #     print('DEBUG validate: type(l)=', type(l))
                #     if isinstance(l, dict):
                #         for k, v in l.items():
                #             if isinstance(v, torch.Tensor):
                #                 print(f"DEBUG validate: l['{k}'] tensor shape={tuple(v.shape)} dtype={v.dtype} device={v.device}")
                #             else:
                #                 print(f"DEBUG validate: l['{k}'] type={type(v)} repr={str(v)[:200]}")
                #     elif isinstance(l, torch.Tensor):
                #         print(f"DEBUG validate: l tensor shape={tuple(l.shape)} dtype={l.dtype} device={l.device}")
                #     else:
                #         print('DEBUG validate: l repr:', str(l)[:200])
                # except Exception as _e:
                #     print('DEBUG validate: error while printing l:', _e)
    ...
    # 計算方法は元のvalidateと同じ、今のvalidateはtrainと同じloss計算方法
    def test(self, validation_loaders, model, epoch, step):
        # print("validate: entered")
        all_matircs = {}
        model.eval()
        for name, loader in validation_loaders.items():
